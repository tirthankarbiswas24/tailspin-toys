...
- Define and document a `Props` interface in frontmatter for every reusable component; describe non-obvious properties and contracts.
...
- Comment intent and non-obvious decisions, not markup or expressions that are already self-explanatory. Keep comments current.
